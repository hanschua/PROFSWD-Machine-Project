function validateFileParallel(file) {
	var extension = file.name.substring(file.name.lastIndexOf('.'));
	if(extension.toLowerCase() != '.parallel') {
      ...
      r.onload = function(e) { 
		...
		for(var i = 0 ; i < lines.length-1; i+=4) {
			...
			//document.getElementById(id).style.top = locationTop + "px";
				...
				loadParallelView(source, target, locationTop, locationLeft, value, label);
			}
				...
				var label = lines[i+3];
				connectLoad(source, target, label);
			}
